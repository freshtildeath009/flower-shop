...
export default function SunflowerProductCheckout() {
  ...
  const [selectedSunflower, setSelectedSunflower] = useState<ProductData>({
    id: 3,
    name: "Sunflower Stand",
    type: `${selectedOptions.color} Sunflowers`,
  ...
  const handleColorChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedSunflower(prev => ({
      ...prev,
      type: `${e.target.value} Sunflowers`
    }));
  };
  ...
  const calculateTotal = (): number => {
    const selectedSizePrice = selectedSunflower.options.sizes.find(
      size => size.name === selectedOptions.size
    )?.price || 0;

    const addonsTotal = selectedOptions.addons.reduce((total, addonName) => {
      const addon = selectedSunflower.options.addons.find(a => a.name === addonName);
      return total + (addon?.price || 0);
    }, 0);

    return (selectedSizePrice + addonsTotal) * selectedSunflower.quantity;
  };
    ...
    try {
      ...
      const total = calculateTotal();
       ...
       const queryParams = new URLSearchParams({
        product: selectedSunflower.name,
        size: selectedOptions.size,
        color: selectedOptions.color,
        addons: JSON.stringify(selectedOptions.addons),
        quantity: selectedSunflower.quantity.toString(),
  ...
  return (
    <div className="bg-rose-200 min-h-screen">
      <Head>
        <title>Sunflower Stand - Jacinto's FlowerShop</title>
        <meta name="description" content="Order beautiful premium sunflower stands from Jacinto's FlowerShop" />
      </Head>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl">
            <h2 className="text-2xl font-semibold text-pink-600 mb-4">
              {selectedSunflower.name}
            </h2>
            <p className="text-gray-600 mb-4">{selectedSunflower.description}</p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                >
                  {selectedSunflower.options.colors.map((color) => (
              ...
              <div>
                ...
                >
                  {selectedSunflower.options.sizes.map((size) => (
              ...
              <div>
                ...
                {selectedSunflower.options.addons.map((addon) => (
            ...
            <div className="space-y-2 mb-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-pink-800">
                  ${selectedSunflower.options.sizes.find(size => size.name === selectedOptions.size)?.price}
                </span>
              </div>
