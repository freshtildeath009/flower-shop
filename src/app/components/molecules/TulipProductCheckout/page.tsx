...
export default function TulipProductCheckout() {
  ...
  const [selectedTulip, setSelectedTulip] = useState<ProductData>({
    id: 2,
    name: "Tulip Bundle",
    type: `${selectedOptions.color} Tulips`,
  ...
  const handleColorChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedTulip(prev => ({
      ...prev,
      type: `${e.target.value} Tulips`
    }));
  };
  ...
  const calculateTotal = (): number => {
    const selectedSizePrice = selectedTulip.options.sizes.find(
      size => size.name === selectedOptions.size
    )?.price || 0;

    const addonsTotal = selectedOptions.addons.reduce((total, addonName) => {
      const addon = selectedTulip.options.addons.find(a => a.name === addonName);
      return total + (addon?.price || 0);
    }, 0);

    return (selectedSizePrice + addonsTotal) * selectedTulip.quantity;
  };
    ...
    try {
      ...
      const total = calculateTotal();
       ...
       const queryParams = new URLSearchParams({
        product: selectedTulip.name,
        size: selectedOptions.size,
        color: selectedOptions.color,
        addons: JSON.stringify(selectedOptions.addons),
        quantity: selectedTulip.quantity.toString(),
  ...
  return (
    <div className="bg-rose-200 min-h-screen">
      <Head>
        <title>Tulip Bundle - Jacinto's FlowerShop</title>
        <meta name="description" content="Order beautiful premium tulip bundles from Jacinto's FlowerShop" />
      </Head>
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl">
            ...
            <p className="text-gray-600 mb-4">{selectedTulip.description}</p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                >
                  {selectedTulip.options.colors.map((color) => (
              ...
              <div>
                ...
                >
                  {selectedTulip.options.sizes.map((size) => (
              ...
              <div>
                ...
                {selectedTulip.options.addons.map((addon) => (
            ...
            <div className="space-y-2 mb-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-pink-800">
                  ${selectedTulip.options.sizes.find(size => size.name === selectedOptions.size)?.price}
                </span>
              </div>
