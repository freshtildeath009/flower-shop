...
export default function LilyProductCheckout() {
  ...
  const [selectedLily, setSelectedLily] = useState<ProductData>({
    id: 5,
    name: "Lilies",
    type: `${selectedOptions.color} Lilies`,
  ...
  const handleColorChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedLily((prev: ProductData) => ({
      ...prev,
      type: `${e.target.value} Lilies`
    }));
  };
  ...
  const calculateTotal = (): number => {
    const selectedSizePrice = selectedLily.options.sizes.find(
      size => size.name === selectedOptions.size
    )?.price || 0;

    const addonsTotal = selectedOptions.addons.reduce((total, addonName) => {
      const addon = selectedLily.options.addons.find(a => a.name === addonName);
      return total + (addon?.price || 0);
    }, 0);

    return (selectedSizePrice + addonsTotal) * selectedLily.quantity;
  };
    ...
    try {
      ...
      const total = calculateTotal();
       ...
       const queryParams = new URLSearchParams({
        product: selectedLily.name,
        size: selectedOptions.size,
        color: selectedOptions.color,
        addons: JSON.stringify(selectedOptions.addons),
        quantity: selectedLily.quantity.toString(),
  ...
  return (
    <div className="bg-rose-200 min-h-screen">
      <Head>
        <title>Lilies - Jacinto's FlowerShop</title>
        <meta name="description" content="Order beautiful premium lilies from Jacinto's FlowerShop" />
      </Head>
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl">
            ...
            <p className="text-gray-600 mb-4">{selectedLily.description}</p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                >
                  {selectedLily.options.colors.map((color) => (
              ...
              <div>
                ...
                >
                  {selectedLily.options.sizes.map((size) => (
              ...
              <div>
                ...
                {selectedLily.options.addons.map((addon) => (
            ...
            <div className="space-y-2 mb-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-pink-800">
                  ${selectedLily.options.sizes.find(size => size.name === selectedOptions.size)?.price}
                </span>
              </div>
