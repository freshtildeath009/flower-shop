...
export default function OrchidProductCheckout() {
  ...
  const [selectedOrchid, setSelectedOrchid] = useState<ProductData>({
    id: 4,
    name: "Orchid Pot",
    type: `${selectedOptions.color} Orchids`,
    ...
    options: {
      ...
      sizes: [
        { name: "Small (12 stems)", price: 19 },
        { name: "Medium (24 stems)", price: 39 },
        { name: "Large (36 stems)", price: 59 }
      ],
      addons: [
        { name: "Vase", price: 15 },
  ...
  const handleColorChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedOrchid(prev => ({
      ...prev,
      type: `${e.target.value} Orchids`
    }));
  };
  ...
  const calculateTotal = (): number => {
    const selectedSizePrice = selectedOrchid.options.sizes.find(
      size => size.name === selectedOptions.size
    )?.price || 0;

    const addonsTotal = selectedOptions.addons.reduce((total, addonName) => {
      const addon = selectedOrchid.options.addons.find(a => a.name === addonName);
      return total + (addon?.price || 0);
    }, 0);

    return (selectedSizePrice + addonsTotal) * selectedOrchid.quantity;
  };
    ...
    try {
      ...
      const total = calculateTotal();
       ...
       const queryParams = new URLSearchParams({
        product: selectedOrchid.name,
        size: selectedOptions.size,
        color: selectedOptions.color,
        addons: JSON.stringify(selectedOptions.addons),
        quantity: selectedOrchid.quantity.toString(),
  ...
  return (
    <div className="bg-rose-200 min-h-screen">
      <Head>
        <title>Orchid Pot - Jacinto's FlowerShop</title>
        <meta name="description" content="Order beautiful premium orchid pots from Jacinto's FlowerShop" />
      </Head>
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl">
            ...
            <p className="text-gray-600 mb-4">{selectedOrchid.description}</p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                >
                  {selectedOrchid.options.colors.map((color) => (
              ...
              <div>
                ...
                >
                  {selectedOrchid.options.sizes.map((size) => (
              ...
              <div>
                ...
                {selectedOrchid.options.addons.map((addon) => (
            ...
            <div className="space-y-2 mb-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-pink-800">
                  ${selectedOrchid.options.sizes.find(size => size.name === selectedOptions.size)?.price}
                </span>
              </div>
