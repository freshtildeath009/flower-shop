...
export default function CarnationProductCheckout() {
  ...
  const [selectedCarnation, setSelectedCarnation] = useState<ProductData>({
    id: 7,
    name: "Carnation Delight",
    type: `${selectedOptions.color} Carnations`,
  ...
  const calculateTotal = (): number => {
    const selectedSizePrice = selectedCarnation.options.sizes.find(
      size => size.name === selectedOptions.size
    )?.price || 0;

    const addonsTotal = selectedOptions.addons.reduce((total, addonName) => {
      const addon = selectedCarnation.options.addons.find(a => a.name === addonName);
      return total + (addon?.price || 0);
    }, 0);

    return (selectedSizePrice + addonsTotal) * selectedCarnation.quantity;
  };
    ...
    try {
      ...
      const total = calculateTotal();
       ...
       const queryParams = new URLSearchParams({
        product: selectedCarnation.name,
        size: selectedOptions.size,
        color: selectedOptions.color,
        addons: JSON.stringify(selectedOptions.addons),
        quantity: selectedCarnation.quantity.toString(),
  ...
  return (
    <div className="bg-rose-200 min-h-screen">
      <Head>
        <title>Carnation Delight - Jacinto's FlowerShop</title>
        <meta name="description" content="Order beautiful premium carnation delights from Jacinto's FlowerShop" />
      </Head>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl">
            <h2 className="text-2xl font-semibold text-pink-600 mb-4">
              {selectedCarnation.name}
            </h2>
            <p className="text-gray-600 mb-4">{selectedCarnation.description}</p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                >
                  {selectedCarnation.options.colors.map((color) => (
              ...
              <div>
                ...
                >
                  {selectedCarnation.options.sizes.map((size) => (
              ...
              <div>
                ...
                {selectedCarnation.options.addons.map((addon) => (
            ...
            <div className="space-y-2 mb-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-pink-800">
                  ${selectedCarnation.options.sizes.find(size => size.name === selectedOptions.size)?.price}
                </span>
              </div>
