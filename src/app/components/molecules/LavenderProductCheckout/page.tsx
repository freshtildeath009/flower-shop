...
export default function LavenderProductCheckout() {
  ...
  const [selectedLavender, setSelectedLavender] = useState<ProductData>({
    id: 8,
    name: "Lavender",
    type: `${selectedOptions.color} Lavenders`,
    ...
    options: {
      ...
      sizes: [
        { name: "Small (12 stems)", price: 20 },
        { name: "Medium (24 stems)", price: 40 },
        { name: "Large (36 stems)", price: 60 }
      ],
      addons: [
        { name: "Vase", price: 15 },
  ...
  const handleColorChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedLavender(prev => ({
      ...prev,
      type: `${e.target.value} Lavenders`
    }));
  };
  ...
  const calculateTotal = (): number => {
    const selectedSizePrice = selectedLavender.options.sizes.find(
      size => size.name === selectedOptions.size
    )?.price || 0;

    const addonsTotal = selectedOptions.addons.reduce((total, addonName) => {
      const addon = selectedLavender.options.addons.find(a => a.name === addonName);
      return total + (addon?.price || 0);
    }, 0);

    return (selectedSizePrice + addonsTotal) * selectedLavender.quantity;
  };
    ...
    try {
      ...
      const total = calculateTotal();
       ...
       const queryParams = new URLSearchParams({
        product: selectedLavender.name,
        size: selectedOptions.size,
        color: selectedOptions.color,
        addons: JSON.stringify(selectedOptions.addons),
        quantity: selectedLavender.quantity.toString(),
  ...
  return (
    <div className="bg-rose-200 min-h-screen">
      <Head>
        <title>Lavender - Jacinto's FlowerShop</title>
        <meta name="description" content="Order beautiful premium lavenders from Jacinto's FlowerShop" />
      </Head>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl">
            <h2 className="text-2xl font-semibold text-pink-600 mb-4">
              {selectedLavender.name}
            </h2>
            <p className="text-gray-600 mb-4">{selectedLavender.description}</p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                >
                  {selectedLavender.options.colors.map((color) => (
              ...
              <div>
                ...
                >
                  {selectedLavender.options.sizes.map((size) => (
              ...
              <div>
                ...
                {selectedLavender.options.addons.map((addon) => (
            ...
            <div className="space-y-2 mb-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-pink-800">
                  ${selectedLavender.options.sizes.find(size => size.name === selectedOptions.size)?.price}
                </span>
              </div>
