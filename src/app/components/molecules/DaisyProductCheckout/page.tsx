...
export default function DaisyProductCheckout() {
  ...
  const [selectedDaisy, setSelectedDaisy] = useState<ProductData>({
    id: 6,
    name: "Daisy Mix",
    type: `${selectedOptions.color} Daisies`,
  ...
  const calculateTotal = (): number => {
    const selectedSizePrice = selectedDaisy.options.sizes.find(
      size => size.name === selectedOptions.size
    )?.price || 0;

    const addonsTotal = selectedOptions.addons.reduce((total, addonName) => {
      const addon = selectedDaisy.options.addons.find(a => a.name === addonName);
      return total + (addon?.price || 0);
    }, 0);

    return (selectedSizePrice + addonsTotal) * selectedDaisy.quantity;
  };
    ...
    try {
      ...
      const total = calculateTotal();
       ...
       const queryParams = new URLSearchParams({
        product: selectedDaisy.name,
        size: selectedOptions.size,
        color: selectedOptions.color,
        addons: JSON.stringify(selectedOptions.addons),
        quantity: selectedDaisy.quantity.toString(),
  ...
  return (
    <div className="bg-rose-200 min-h-screen">
      <Head>
        <title>Daisy Mix - Jacinto's FlowerShop</title>
        <meta name="description" content="Order beautiful premium daisy mix from Jacinto's FlowerShop" />
      </Head>
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl">
            ...
            <p className="text-gray-600 mb-4">{selectedDaisy.description}</p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                >
                  {selectedDaisy.options.colors.map((color) => (
              ...
              <div>
                ...
                >
                  {selectedDaisy.options.sizes.map((size) => (
              ...
              <div>
                ...
                {selectedDaisy.options.addons.map((addon) => (
            ...
            <div className="space-y-2 mb-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-pink-800">
                  ${selectedDaisy.options.sizes.find(size => size.name === selectedOptions.size)?.price}
                </span>
              </div>
